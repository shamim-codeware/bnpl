...
class BnplOrdersExport implements FromCollection, WithHeadings, WithMapping, WithEvents
{
    ...
    //     // Determine if we should zero numeric fields
    //     $zeroFields = in_array($purchase->status, [2, 4]); // 2 = Rejected, 4 = Sale Cancel

    //     $outstanding_balance = $zeroFields ? 0 : Installment::where('hire_purchase_id', $purchase->id)
    //         ->where('status', 0)
    //         ->sum('amount');

    //     $first_installment = $zeroFields ? 0 : ($purchase->purchase_product->down_payment ?? 0);
    //     $total_paid = $zeroFields ? 0 : ($purchase->purchase_product->total_paid ?? 0);
    //     $paid_installments = $zeroFields ? 0 : (@$purchase->installment ? $purchase->installment->where('status', 1)->count() : 0);
    //     $due_installments  = $zeroFields ? 0 : (@$purchase->installment ? $purchase->installment->where('status', 0)->count() : 0);
    //     $last_paid_amount = $zeroFields ? 0 : $last_paid_amount;
    ...
    //     //     $purchase->order_no,
    //     //     $firstLoanStartDate ? \App\Helpers\Helper::formatDateStandard($firstLoanStartDate) : 'N/A',
    //     //     $lastLoanEndDate ? \App\Helpers\Helper::formatDateStandard($lastLoanEndDate) : 'N/A',
    //     //     @$purchase->purchase_product->brand->name ?? 'N/A',
    //     //     @$purchase->purchase_product->product->product_model ?? 'N/A',
    //     //     // @$purchase->purchase_product->product_size->name ?? 'N/A',
    ...
    //     //     @$purchase->purchase_product ? number_format($purchase->purchase_product->down_payment, 2) : '0.00',
    //     //     @$purchase->purchase_product ? number_format($purchase->purchase_product->monthly_installment, 2) : '0.00',
    //     //     @$purchase->purchase_product ? number_format($purchase->purchase_product->total_paid, 2) : '0.00',
    //     //     number_format($outstanding_balance, 2),
    //     //     @$purchase->installment ? $purchase->installment->count() : '0',
    //     //     @$purchase->installment ? $purchase->installment->where('status', 1)->count() : '0',
    //     //     @$purchase->installment ? $purchase->installment->where('status', 0)->count() : '0',
    //     //     number_format($last_paid_amount, 2),
    //     //     $last_payment_date ? \App\Helpers\Helper::formatDateTimeStandard($last_payment_date) : 'N/A',
    ...
    //         @$purchase->purchase_product->product_size_id ?? 'N/A',
    //         @$purchase->purchase_product ? number_format($purchase->purchase_product->hire_price, 2) : '0.00',
    //         number_format($first_installment, 2),
    //         @$purchase->purchase_product ? number_format($purchase->purchase_product->monthly_installment, 2) : '0.00',
    ...
    public function map($purchase): array
{
    ...
    $firstInstallment = $isRejectedOrCancelled ? '0.00' :
        (@$purchase->purchase_product ? number_format($purchase->purchase_product->down_payment, 2) : '0.00');

    $totalPaymentReceived = $isRejectedOrCancelled ? '0.00' :
        (@$purchase->purchase_product ? number_format($purchase->purchase_product->total_paid, 2) : '0.00');

    $outstandingBalanceFormatted = $isRejectedOrCancelled ? '0.00' :
        number_format($outstanding_balance, 2);

    $paidInstallment = $isRejectedOrCancelled ? '0' :
        (@$purchase->installment ? $purchase->installment->where('status', 1)->count() : '0');

    $dueInstallment = $isRejectedOrCancelled ? '0' :
        (@$purchase->installment ? $purchase->installment->where('status', 0)->count() : '0');
