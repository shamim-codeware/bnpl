...
<head>
    <meta charset="utf-8">
    <title>Guarantor Reminder</title>
...
<body>
    <div class="formBody">
            ...
            <tr>
                ...
                <td style="width: 36%;">
                    <table style="width:100%; font-size:14px; text-align:left; border: 1px solid #000;">
                        <tr>
                            <td colspan="2"
                                style="background:#e30613; color:#fff; font-weight:bold; text-align:center; padding:3px; font-size:14px;">
                                Product Purchase Information
                            </td>
                        </tr>
                        <tr>
                            <td
                                style="border:1px solid #000; padding:3px; background: #F2F2F2; font-weight:bold; font-size:12px;">
                                Date
                            </td>
                            <td style="border:1px solid #000; padding:3px; font-size:12px;">
                ...
                <td style="width: 26%;">
                    <table style="width:100%;" cellspacing="0">
                        <tr>
                            <td
                                style="font-size:12px; border:1.5px solid #000; height:200px; text-align:center; vertical-align:middle; padding:15px;">
                                গ্যারান্টর<br>ছবি
                            </td>
                        </tr>
                    </table>
        ...
        <table style="margin-top:15px;">
            <tr>
                <td style="text-align:left; font-weight:bold; font-size: 15px;">
                    বিষয়: বকেয়া কিস্তি পরিশোধের অনুরোধ - গ্যারান্টর রিমাইন্ডার
                </td>
            </tr>
        </table>
        ...
        {{-- <p style="margin-bottom: 15px;">
            ...
            র‍্যাংগস ইলেকট্রনিক্স থেকে উপরে উল্লেখিত তথ্যমতে একটি
            <strong>RANGS GOOGLE TV</strong>
            মডেল
            <strong>RG43ZX</strong>
            সিরিয়াল নম্বর
            <strong>RG43ZKT0425024</strong>,
            যার সর্বমোট কিস্তি মূল্য
            <strong>৩৯,৮০৫ টাকা</strong>
            (ঊনচল্লিশ হাজার আট শত পাঁচ টাকা),
            ৩ টি কিস্তিতে ক্রয় করেছেন।
        </p> --}}

        @php
            $name = $penalty->installment->hire_purchase->name;
            $words = explode(' ', $name);

            foreach ($words as $index => $word) {
                if ($index === 0) {
                    // প্রথম শব্দ শুধু প্রথম অক্ষর বড়, বাকিটা ছোট
                    $words[$index] = ucfirst(strtolower($word));
                } else {
                    // পরের শব্দগুলো: প্রথম অক্ষর বড়, বাকিটা ছোট
                    $words[$index] = ucfirst(strtolower($word));
                }
            }
        ...
        <p style="margin-bottom: 15px;">
            ...
            র‍্যাংগস ইলেকট্রনিক্স থেকে উপরে উল্লেখিত তথ্যমতে একটি
            {{ $penalty->installment->hire_purchase->purchase_products->pluck('product_category.name')->implode(', ') }}</strong>,
            মডেল
            <strong>{{ $penalty->installment->hire_purchase->purchase_products->pluck('product.product_model')->implode(', ') }}</strong>,
            সিরিয়াল নম্বর
            <strong>{{ $penalty->installment->hire_purchase->purchase_products->pluck('serial_no')->implode(', ') }}</strong>,
            সর্বমোট কিস্তি মূল্য
            <strong>{{ \App\Helpers\Helper::en_to_bn_number(number_format($penalty->installment->hire_purchase->hire_price, 2, '.', ',')) }}</strong>
            ({{ \App\Helpers\Helper::numberToBengaliWords($penalty->installment->hire_purchase->hire_price) }})
            {{ \App\Helpers\Helper::en_to_bn_number($penalty->installment->hire_purchase->installment_month) }} টি
            কিস্তিতে ক্রয় করেছেন।
        </p>

        <p style="margin-bottom: 15px;">
            বর্তমানে গ্রাহকের নামে কোম্পানির সাথে বিক্রয় চুক্তির সিডিউল অনুসারে
            <strong>
                ২ (দুই) টি কিস্তির
                ({{ \App\Helpers\Helper::en_to_bn_number(number_format($penalty->installment_amount, 2, '.', ',')) }} ×
                ২) =
                {{ \App\Helpers\Helper::en_to_bn_number(number_format($penalty->installment_amount * 2, 2, '.', ',')) }}
                টাকা ({{ \App\Helpers\Helper::numberToBengaliWords($penalty->installment_amount * 2) }}) বকেয়া
                রয়েছে,</strong>
            যা পরিশোধের জন্য ইতিপূর্বে পত্রের মাধ্যমে
            <strong>(Ref: REL/BNPL/RLBNPLR00686-01)</strong>
            এবং মৌখিকভাবে যোগাযোগ করা হয়েছে।
        ...
        <p style="margin-bottom: 15px;">
            চুক্তি অনুসারে
            <strong>২ নং</strong>
            কিস্তির জন্য বিলম্ব <strong>ফি ৫০০</strong> টাকা
            পাওনা হয়েছে।
            এর ফলে সর্বমোট বকেয়া পরিমাণ
            <strong>({{ \App\Helpers\Helper::en_to_bn_number(number_format($penalty->installment_amount * 2, 2, '.', ',')) }}
                + ৫০০) =
                {{ \App\Helpers\Helper::en_to_bn_number(number_format($penalty->installment_amount * 2 + 500, 2, '.', ',')) }}
                টাকা</strong>।
        </p>
        ...
        <p style="margin-bottom: 15px;">
            ...
            গ্যারান্টর হিসেবে কিস্তি চুক্তি অনুযায়ী
            সমুদয় বকেয়া মূল্য/কিস্তি প্রদানে সম্মত হয়েছেন।
        </p>

        <p>
            গ্যারান্টর হিসেবে আপনাকেও বিষয়টি অবহিত করা হলো
            যাতে আপনি গ্রাহককে বকেয়া পরিশোধের বিষয়ে সচেতন করেন
            এবং কিস্তিটি দ্রুত নিষ্পত্তিতে সহায়তা করেন।
        </p>
        ...
        <table style="margin-top:35px;">
            <tr>
                ...
                <td
                    style="text-align: center; padding-top:5px; border-top:1.5px dashed #000; vertical-align:bottom; padding-bottom: 20px;">
